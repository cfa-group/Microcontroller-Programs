...
void setup() {
  ...
  tft.drawString("MPU-6050 Project", 150, 10, 4);
  //tft.drawString("Weather", 350, 10, 2);
 
  tft.drawString("Front-Back", 80, 270, 2);
  tft.drawString("Left-Right", 320, 270, 2);
...
void loop() {
  ...
  if ((millis() - mputime ) > 1000){
    Serial.println("Read MPU-Module");
    Read_MPU_Module();
    mputime = millis();
    }
  ...
  if (millis() - runTime >= 0){
   ...
   //reading = 800 + 150 * sineWave(d + 90);
    xpos = gap + ringMeter(gyro_x, 0, 100, xpos, ypos, radius, "%", BLUE2RED); // Draw analogue meter
 
    //reading = 15 + 15 * sineWave(d + 150);
    xpos = gap + ringMeter(gyro_y, 0, 100, xpos, ypos, radius, "%", BLUE2RED); // Draw analogue meter
  }
}
...
void Read_MPU_Module(){
  ...
  if (_x < 0.00 || _y < 0.00){
    gyro_x = map(_x, -90, 0, -100, 0);
    gyro_y = map(_y, -90, 0, -100, 0);
    }

  if (_x < 0.00 || _y < 0.00){
    gyro_x = map(_x, 0, 90, 0, 100);
    gyro_y = map(_y, 0, 90, 0, 100);
  }

  Serial.print("maping X: ");
  Serial.print(gyro_x);
  Serial.print(", Y: ");
